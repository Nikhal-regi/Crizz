...
const PlayerScorecard = ({ batsmanStats, bowlerStats }) => {
  return (
    <div className="max-w-lg mt-10 mx-auto bg-[#c4e5c3] shadow-md rounded-lg p-6">
      ...
      <div className="mb-6 text-left">
        ...
        <div className="grid grid-cols-3 gap-4 text-left font-semibold">
          <span>Name</span>
          <span>Runs</span>
          <span>Balls</span>
        </div>
        {batsmanStats.length === 0 ? (
          <p className="text-left text-gray-600">No batsman data available</p>
        ) : (
          batsmanStats.map((batsman, index) => (
            <div
              key={index}
              className="grid grid-cols-3 gap-4 text-left text-gray-700 py-2 border-b"
            >
      ...
      <div>
        ...
        {bowlerStats.length === 0 ? (
          <p className="text-left text-gray-600">No bowler data available</p>
        ) : (
          bowlerStats.map((bowler, index) => (
            <div
              key={index}
              className="grid grid-cols-5 gap-4 text-left text-gray-700 py-2 border-b"
            >
